...
The browser client is real Phoenix + Phoenix LiveView JavaScript, served
same-origin (no bundler, no CDN):
...
They are generated reproducibly from the already-fetched deps — never
committed and never downloaded — by:

```bash
mise x -- mix assets.setup    # copies the pinned dists into vendor/
```

`mix setup`, `mix test` and `mix precommit` run this automatically, so a
clean checkout has the files after the documented commands. If a page ever
