defmodule Triage.ReviewIntegrations do
  ...
  def team_mapping(owner) do
    case Map.get(Keyword.get(config(), :teams, %{}), owner) do
      ...
      when is_binary(project) and project != "" and is_binary(area) and area != "" ->
        mapping
  ...
  def create_ticket(plan) do
    config = Keyword.put(config(), :organization, plan.organization)
  ...
  def validate_advice(output) do
    case Jason.decode(output) do
      ...
      when recommendation in ["whitelist", "fix", "investigate"] and is_binary(reason) and
             byte_size(reason) > 0 and byte_size(reason) <= 8000 ->
        {:ok, %{recommendation: recommendation, reason: reason}}

      _ ->
        {:error, "Internal AI returned an invalid recommendation."}
    end
  end

  defmodule Azure do
    @moduledoc "Azure DevOps adapter for explicitly confirmed remediation plans."
    def create(plan, config) do
      ...
      url =
        "https://dev.azure.com/#{segment(org)}/#{segment(project)}/_apis/wit/workitems/$#{segment(type)}?api-version=7.1"
      ...
      body = [
        ...
        %{
          ...
          value: "<pre>#{escape(plan.description)}</pre>"
        }
      ]

      case Req.post(url,
             auth: {:basic, ":" <> Keyword.fetch!(config, :token)},
        ...
        when status in 200..299 and is_integer(id) and id > 0 ->
          {:ok, id,
           "https://dev.azure.com/#{segment(org)}/#{segment(project)}/_workitems/edit/#{id}"}

        {:ok, %{status: status}} when status in 400..499 and status != 408 ->
          {:error, :rejected}
