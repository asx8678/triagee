defmodule TriageWeb.WorkspaceComponents do
  ...
  def overview(assigns) do
    ...
    <div class="dashboard-grid">
      <section class="panel">
        <div class="panel-head">
          <div>
            <h2>Ownership by team</h2><p>Who needs to act, not just who has findings.</p>
          </div><span class="tag">Active CVEs</span>
        </div>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            <h2>Focus next</h2><p>A short, actionable queue.</p>
          </div><span class="tag">Prioritized</span>
        </div>
        <div
          :for={
            {mode, title, copy} <- [
        ...
        >
          <div class="number">{@metrics[mode].value}</div><div class="grow">
            <h3>{title}</h3><p>{copy}</p>
          </div><.link patch={Routes.drill(@params, mode)} aria-label={title}>→</.link>
        </div>
      </section>
    </div>
    <div class="dashboard-secondary">
      ...
      <section class="panel">
        <div class="panel-head">
          <h2>Evidence health</h2><button class="link" phx-click="settings">Details →</button>
        </div><div class="panel-body">
          <div class="evidence-row">
            <span class="label">Recorded affected scopes</span><strong>{length(
              @metrics["active"].targets
            )}</strong>
          </div><div class="evidence-row">
            <span class="label">Scan coverage / freshness</span><span class="amber">Unverified</span>
          </div><div class="evidence-row">
            <span class="label">Public-reference advisories</span><span>Excluded</span>
          </div><div class="evidence-row">
            <span class="label">Reading / local save</span><span>No external operations</span>
          </div>
        </div>
      </section>
    </div>
        ...
        <table id="workspace-inventory" class="data-table">
          <caption class="sr-only">
            ...
            <tr
              ...
              class={[row.cve in @selected && "selected"]}
            >
              <td>
                <input
                  type="checkbox"
      ...
      >
        <header class="review-heading">
          <div class="between">
            ...
            </div><.link patch={Routes.inspector_path(@params, @row.cve)}>Full evidence ↗</.link>
          </div>
        </header>
        <.form
          for={@form}
        ...
        >
          ...
          <div class="decision-column">
            ...
            <.input
              ...
              type="textarea"
              label={
                if @draft.fields["action"] == "request_verification",
                  do: "Claimed evidence / work reference and rationale",
                  else: "Rationale / controls / question"
              }
              maxlength="2000"
            />
            <p class="form-note">
        ...
        <footer class="review-footer">
          <span id="draft-state" class="save-state">{if @draft.saved,
            ...
            <button
              ...
              disabled={@hidden_targets != []}
            >Save decision</button><button
              id="save-next"
              class="primary"
              type="submit"
              form="workspace-decision"
                ...
                >
                  <time>{time(d.decided_at)}</time><div>
                    <strong>{d.label} · {d.state}</strong><p>{history_scope(d)}</p><p>
                      {d.actor} · self-declared · {d.reason}
                    </p><p :if={d.expires_at}>
      ...
      <div class="confirmation-layout">
        <header class="modal-head">
          ...
          <strong>{@row.cve} · {length(@draft.targets)} exact targets</strong><.scope_table targets={
            ...
            Recorded by {@draft.fields["actor"]} (self-declared). Responsible: {@draft.fields["owner"]}.
