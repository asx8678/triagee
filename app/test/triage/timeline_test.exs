defmodule Triage.TimelineTest do
  ...
  describe "window" do
    test "defaults to eight Monday-aligned weeks ending today" do
      ...
      # Monday to today is a round 56 days only on a Sunday, so the span is
      # derived from the contract rather than hard-coded.
      assert length(view.days) == span.days
      assert span.days == 49 + Date.day_of_week(span.to)

      # The window ends today and is never padded into days that have not
      # happened: unless today closes its week, it is shorter than eight full
      ...
      assert Date.day_of_week(view.window.to) == 7 or length(view.days) < 8 * 7
      ...
      # The label states the real span, so "Last 8 weeks" cannot pass a shorter
      # partial week off as eight full weeks.
      assert view.window.label =~ "Last 8 weeks"
    ...
    test "an empty window reports zero observations without claiming a clean estate" do
      ...
      assert view.summary.empty_days == window_span(8).days
  ...
  describe "day bands and connectors" do
    ...
    test "bands are newest first and empty days are explicit" do
      ...
      assert view.summary.empty_days == window_span(8).days - 2
    end
