defmodule TriageWeb.CaseLiveIndexTest do
  ...
  test "rows render badges, review metadata and saved-scope detail links", %{conn: conn} do
    ...
    assert has_element?(view, "#case-cve-#{alpha.id}[href='/cves/CVE-2025-1001']")
    assert has_element?(view, "#case-cve-#{beta.id}[href='/cves/CVE-2025-1001']")
  end
  ...
  test "URL scope filters rows; row links keep each row's saved scope", %{conn: conn} do
    ...
    assert has_element?(view, "#case-cve-#{alpha.id}[href='/cves/CVE-2025-1001?owner=alpha']")
