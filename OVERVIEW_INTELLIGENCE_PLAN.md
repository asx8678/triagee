...
| 9 Public routes, context APIs, Mix task and configuration mechanically checked | Verified | `mix phx.routes` lists `/`, `/findings`, `/findings/:id`, `/cves/:id`; `mix help triage.intel` documents a cache-only refresh; `config :triage, :intel, enabled: false, sources: []`. |
