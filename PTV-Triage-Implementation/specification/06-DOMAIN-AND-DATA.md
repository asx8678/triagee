# 06 · Domain, count and data contracts

This file adopts the original design's domain requirements below. Read these with the binding clarifications at the end; conceptual fields are integration contracts, not an instruction to replace the application's entire schema.
...
## Binding clarifications for the implementation package

**Scope-local priority.** A CVE's headline priority in a filtered view is the highest applicable priority among matching active targets, under the real versioned policy. It cannot inherit a P1 result from an out-of-scope production deployment while showing staging only. The sample reference stores one priority per advisory for demonstration; this is not the production algorithm.

**Inclusive expiration.** For an exception valid through a chosen date in a named timezone, store the exclusive beginning of the following local day as the expiration instant. This avoids fractional-second ambiguity and supports timezone changes correctly. The UI may say “through [date]”; audit records retain both original date/timezone and computed instant. Validate daylight-saving transitions using the repository's timezone facilities.

**Distinct daily counts.** A heatmap labeled first-observed CVEs deduplicates canonical CVE IDs in the selected scope and daily bucket. A per-scope observation event count is a different measure. The chart, day drilldown, summary and event log must expose their units and reconcile by identifiers.

**No out-of-scope fallback.** A CVE inspector finding no matching targets shows the filtered-out state. It does not substitute all targets. Scope-aware history filters events by their original target set and labels legacy/global/unknown-scope events explicitly instead of pretending they belong to every team.

**Metrics with missing coverage.** The active metric is the latest recorded affected state in the selected operational dataset, with freshness/coverage qualification. It is not a verified live detector. Public-reference-only advisories remain outside operational affected totals even if their advisory identifiers are genuine.

**Fixture is not policy.** `contracts/synthetic-fixture.json` and `fixture-expectations.json` freeze the sample for visual QA. Their fixed clock, supplied priority, synthetic package names and intelligence flags are not real CVE intelligence or recommended upgrades. Keep a test-only adapter translating these records into existing domain entities; never seed them into production.

**No required new risk engine.** Use existing well-defined prioritization where it exists. A needed policy refinement requires an explicit versioned contract and tests. Do not infer a real SSVC implementation from the reference's illustrative P1/P2/P3 tags.
